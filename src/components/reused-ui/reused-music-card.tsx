...
export const MusicCard = memo(({ music, onRate, onToggleFavorite, onRemove }: IMusicCard) => {
  ...
  const formattedSize = useMemo(() => {
    return formatSize(music.fileSize);
  }, [music.fileSize]);
  ...
  const copyLink = useCallback(() => {
    navigator.clipboard.writeText(music.youtubeLink);
    setIsMenuOpen(false);
    notify.success("Link copied to clipboard!", 1500);
  }, []);
      ...
      <div className="flex items-center mt-1">
        <div className="flex-1">
          ...
          <div className="flex items-center gap-1">
            <p className="text-[10px] sm:text-sm text-[var(--gray-400)] mt-1">Size: {formattedSize}</p>
          ...
          <div className="flex items-center gap-1 mt-2">
            {[1, 2, 3, 4, 5].map((n) => {
              const isFilled = n <= music.rating;

              const starColor = isFilled ? "var(--yellow-400)" : "var(--gray-400)";
              ...
              return (
                ...
                >
                  <Star size={15} fill={isFilled ? starColor : "none"} stroke={starColor} className="transition" />
                </button>
              );
            })}
          </div>
        ...
        <div ref={cardRef} className="flex relative">
          ...
          <div
            className={` absolute overflow-hidden right-3 -top-[90px] w-32 min-w-[140px] z-50 bg-[var(--gray-800)] rounded-md shadow-lg flex flex-col text-xs text-left text-[var(--gray-100)] transform duration-200 ease-in-out ${
