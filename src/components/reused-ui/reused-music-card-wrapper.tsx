...
export const MusicCardWrapper = memo(
  ({ musicList, sectionTitle }: IMusicCardWrapper) => {
    const [sortOption, setSortOption] = useState<SortOptions>("");
    const removeFromConverted = useConvertedSong(
      (state) => state.removeFromConverted
    );

    const handleRemoveList = () => {
      if (musicList.length === 0) {
        notify.info("Music list is already empty", 2000);
        return;
      }
      uycmusic.destroy();
      notify.warning("Music list cleared", 2000);
    };
    ...
    const handleRate = useCallback((rating: number, music: Music) => {
      ...
      notify.success(
        `Rated ${music.title} with ${rating} star${rating > 1 ? "s" : ""}`,
        2500
      );
    }, []);
    ...
    return (
      <>
        ...
        <div className="mb-2 flex justify-start items-center text-xs sm:text-lg font-semibold gap-4">
          ...
          <div>
            <button className="" onClick={handleRemoveList}>
              Clear music list
            </button>
          </div>
        </div>
